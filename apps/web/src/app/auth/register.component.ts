      ...
      <div class="auth-card">
        <div class="brand">
          <svg class="logo-icon" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <linearGradient id="rgn" x1="0" y1="0" x2=".3" y2="1"><stop offset="0%" stop-color="#22c55e"/><stop offset="100%" stop-color="#059669"/></linearGradient>
              <linearGradient id="rbl" x1=".7" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#3b82f6"/><stop offset="100%" stop-color="#1e3a8a"/></linearGradient>
            </defs>
            <circle cx="30" cy="8" r="7" fill="url(#rgn)"/>
            <path d="M30 15C6 28 4 72 40 84" stroke="url(#rgn)" stroke-width="10" stroke-linecap="round" fill="none"/>
            <path d="M40 84L24 80 30 68" stroke="url(#rgn)" stroke-width="6" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
            <circle cx="70" cy="8" r="7" fill="url(#rbl)"/>
            <path d="M70 15C94 28 96 72 60 84" stroke="url(#rbl)" stroke-width="10" stroke-linecap="round" fill="none"/>
            <path d="M60 84L76 80 70 68" stroke="url(#rbl)" stroke-width="6" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
        ...
        <form (ngSubmit)="onSubmit()" class="form">
          <label>
            {{ i18n.t('register.company') }}
            <input
              type="text"
              [(ngModel)]="companyName"
              name="companyName"
              [placeholder]="i18n.t('register.company_placeholder')"
              required
            />
          </label>
          <label>
        ...
        <p class="trial">{{ i18n.t('register.trial') }}</p>
        <p class="link">
          {{ i18n.t('register.has_account') }}
          <a routerLink="/login">{{ i18n.t('register.sign_in') }}</a>
        </p>
      </div>
    </div>
  `,
...
export class RegisterComponent {
  ...
  onSubmit() {
    ...
    this.error.set('');
    this.auth.register(this.companyName, this.email, this.password).subscribe({
